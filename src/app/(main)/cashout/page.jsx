...
export default function Cashout() {
  ...
  const handleCashout = (e) => {
    e.preventDefault();
    if (agent.length < 11 || !amount || pin.length < 4) {
      toast.error("Please fill in all fields correctly.");
      return;
    }

    const formData = new FormData(e.target);
    const cashout = Object.fromEntries(formData.entries());
    console.log(cashout); 

    setLoading(true);
    setTimeout(() => {
      setLoading(false);
      toast.success(`Successfully withdrew ৳${amount} via Agent ${agent}!`);
      setAgent("");
      setAmount("");
      setPin("");
    }, 1500);
      ...
      >
        <form onSubmit={handleCashout}>
          ...
          <div style={{ marginBottom: 20 }}>
            <label style={labelStyle}>Agent Number</label>
            <input
              name="agent"
              type="number"
              placeholder="Enter 11 digit number"
              value={agent}
              onChange={(e) => setAgent(e.target.value.slice(0, 11))}
